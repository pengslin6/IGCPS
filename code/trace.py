...
def load_sr_com_data(csv_path):
    ...
    # Controller-state fields are control-plane telemetry rather than process
    # variables. Keep a separate view for typed-token studies and retain them
    # in the information-layer feature vector used by the established model.
    control_cols = [
        c for c in ['phy_Controller status', 'phy_Controller status value']
        if c in df.columns
    ]
    phy_cols = [c for c in phy_cols if c not in control_cols]
    print(f"📊 独立控制上下文 ({len(control_cols)}): {control_cols}")

    # Identify endpoint columns by exact normalized names first. Aggregate
    # fields such as cyber_unique_src_ip_count must remain numeric features.
    normalized_columns = {
        c: c.lower().strip().replace(' ', '_').replace('-', '_')
        for c in df.columns
    }
    src_exact = {'source_ip', 'src_ip', 'sourceip'}
    dst_exact = {'destination_ip', 'dest_ip', 'dst_ip', 'destinationip', 'destip'}
    src_ip_col = next(
        (c for c, normalized in normalized_columns.items() if normalized in src_exact),
        None,
    )
    dst_ip_col = next(
        (c for c, normalized in normalized_columns.items() if normalized in dst_exact),
        None,
    )

    # If standard names are absent, fall back to conservative substring matching.
    if src_ip_col is None or dst_ip_col is None:
        for c in df.columns:
            c_lower = c.lower()
            if src_ip_col is None and 'ip' in c_lower and (
                'src' in c_lower or 'source' in c_lower
            ) and 'count' not in c_lower:
                src_ip_col = c
            elif dst_ip_col is None and 'ip' in c_lower and (
                'dst' in c_lower or 'dest' in c_lower
            ) and 'count' not in c_lower:
                dst_ip_col = c
    ...
    control_data = df[control_cols].copy() if control_cols else pd.DataFrame(index=df.index)
    for c in control_cols:
        control_data[c] = pd.to_numeric(control_data[c], errors='coerce').fillna(0)
    X_control = (
        control_data.values.astype(np.float32)
        if control_cols else np.zeros((len(df), 0), dtype=np.float32)
    )
...
def subset_data(data, indices):
    ...
    subset = {}
    per_sample_keys = {
        'times', 'X_phy', 'X_control', 'X_net_feat', 'src_ips', 'dst_ips',
        'y', 'labels_raw'
    }
...
def chronological_split_and_scale(data, train_ratio=0.6, val_ratio=0.2, class_aware=True):
    ...
    scaler_control = None
    if train_data.get('X_control') is not None and train_data['X_control'].shape[1] > 0:
        scaler_control = StandardScaler().fit(train_data['X_control'])
...
def _phy_indices_by_columns(graph_builder, column_names):
    """Return physical-node indices for exact released process variables."""
    wanted = {
        str(name).strip().lower().replace('_', '') for name in column_names
    }
    matches = set()
    for local_idx, column in enumerate(graph_builder.phy_cols):
...
def infer_true_root_nodes(sample_idx, data, graph_builder):
    ...
    2) Dataset-specific roots use independently documented devices/variables.
    3) IGCPS physical faults use small process-specific physical-node sets.
    4) Generic network-origin data use Source IP only when no stricter mapping exists.
    """
    ...
    # TE-CUP-SEC publishes exact affected variables only for Attacks 4--8.
    # Attacks 1--3 alter control setpoints that are absent from the released
    # 53 process variables, so strict exact-node evaluation must skip them.
    if label_upper in {'ATTACK1', 'ATTACK2', 'ATTACK3'}:
        return set()
    if label_upper == 'ATTACK4':
        return _phy_indices_by_columns(graph_builder, ['phy_xmv3', 'phy_xmv4'])
    if label_upper == 'ATTACK5':
        return _phy_indices_by_columns(graph_builder, ['phy_xmv10'])
    if label_upper == 'ATTACK6':
        return _phy_indices_by_columns(graph_builder, ['phy_xmv11'])
    if label_upper == 'ATTACK7':
        return _phy_indices_by_columns(graph_builder, ['phy_xmeas15'])
    if label_upper == 'ATTACK8':
    ...
    # The IGCPS attack scripts target PLC 10.0.6.30:102, and the captured
    # initiating S7/TCP flow is consistently 10.0.6.118 -> 10.0.6.30.  The
    # attacker endpoint, not whichever direction an arbitrary packet happens
    # to use, is the concrete root for PM/PS/SS.
    if label_upper in {'PM', 'PS', 'SS'}:
        attacker_node = graph_builder.ip_to_idx.get('10.0.6.118')
...
class HeteroGraphBuilder:
    ...
    def __init__(self, phy_cols, all_ips, net_feat_dim=1, phy_feat_dim=1,
        ...
        self.preserve_net_features = bool(preserve_net_features)
        ...
        # The legacy layout keeps four summary statistics.  The revised
        # layout also preserves every flow feature after the endpoint-role
        # fields, avoiding an information bottleneck before graph encoding.
        required_net_dim = (
            self.net_raw_offset + net_feat_dim
            if self.preserve_net_features else net_feat_dim
        )
        self.node_feat_dim = max(required_net_dim, phy_feat_dim, 8)
    ...
    def get_node_features(self, X_net_feat_t, X_phy_t, src_ip, dst_ip,
        ...
        if src_ip in self.ip_to_idx:
            ...
            if self.preserve_net_features:
                raw_end = self.net_raw_offset + min(len(net_feat_arr), self.net_feat_dim)
                net_feats[src_idx, self.net_raw_offset:raw_end] = net_feat_arr[:raw_end - self.net_raw_offset]
        ...
        if dst_ip in self.ip_to_idx:
            ...
            if self.preserve_net_features:
                raw_end = self.net_raw_offset + min(len(net_feat_arr), self.net_feat_dim)
                net_feats[dst_idx, self.net_raw_offset:raw_end] = net_feat_arr[:raw_end - self.net_raw_offset]
    ...
    def get_node_features_temporal(self, X_net_feat_window, X_phy_window, src_ip, dst_ip,
        ...
        if self.preserve_net_features and K >= 2:
            previous = np.asarray(X_net_feat_window[-2]).flatten()
            width = min(len(previous), len(net_feat_arr), self.net_feat_dim)
            if width > 0:
                delta = net_feat_arr[:width] - previous[:width]
                active_nodes = []
                if src_ip in self.ip_to_idx:
                    active_nodes.append(self.ip_to_idx[src_ip])
                if dst_ip in self.ip_to_idx:
                    active_nodes.append(self.ip_to_idx[dst_ip])
                for node_idx in set(active_nodes):
                    net_feats[node_idx, self.node_feat_dim + 2] = np.mean(delta)
                    net_feats[node_idx, self.node_feat_dim + 3] = np.std(delta)
...
class HGANTraceEncoder(nn.Module):
    ...
    aggregation inside each layer, after which bidirectional cross-attention
    exchanges information between the network and physical node sets. This
    keeps the graph topology sparse while making cyber-physical fusion an
    explicit learned operation.
    """
    ...
    def __init__(self, in_channels: int, hidden_channels: int,
        ...
        self.use_type_adapters = use_type_adapters
        ...
        if use_type_adapters:
            ...
            self.net_type_bias = nn.Parameter(torch.zeros(1, hidden_channels))
            self.phy_type_bias = nn.Parameter(torch.zeros(1, hidden_channels))
            self.type_norm = nn.LayerNorm(hidden_channels)

        self.neighbor_fusion = nn.ModuleList([
            nn.Linear(hidden_channels * 2, hidden_channels)
        ...
        if use_cross_attention:
            ...
            self.net_gate = nn.Sequential(
                nn.Linear(hidden_channels * 2, hidden_channels), nn.Sigmoid()
            )
            self.phy_gate = nn.Sequential(
                nn.Linear(hidden_channels * 2, hidden_channels), nn.Sigmoid()
            )
            self.net_cross_norm = nn.LayerNorm(hidden_channels)
            self.phy_cross_norm = nn.LayerNorm(hidden_channels)
    ...
    def encode(self, x, adj, n_net=None):
        h = self.input_proj(x)
        n_net = int(n_net or 0)
        n_net = min(max(n_net, 0), h.shape[0])

        if self.use_type_adapters and 0 < n_net < h.shape[0]:
            net = h[:n_net]
            phy = h[n_net:]
            net = net + self.net_adapter(net) + self.net_type_bias
            phy = phy + self.phy_adapter(phy) + self.phy_type_bias
            h = self.type_norm(torch.cat([net, phy], dim=0))

        # Self-loops preserve each node's own evidence during neighborhood fusion.
        adj_local = adj + torch.eye(adj.shape[0], dtype=adj.dtype, device=adj.device)
        degree = adj_local.sum(dim=1, keepdim=True).clamp(min=1.0)
        adj_norm = adj_local / degree
        for layer, norm in zip(self.neighbor_fusion, self.local_norms):
            neighbor = torch.matmul(adj_norm, h)
            update = layer(torch.cat([h, neighbor], dim=-1))
            update = F.gelu(update)
            update = F.dropout(update, p=self.dropout, training=self.training)
            h = norm(h + update)

        if self.use_cross_attention and 0 < n_net < h.shape[0]:
            net = h[:n_net].unsqueeze(0)
            phy = h[n_net:].unsqueeze(0)
            net_msg, _ = self.net_from_phy(net, phy, phy, need_weights=False)
            phy_msg, _ = self.phy_from_net(phy, net, net, need_weights=False)
            net_gate = self.net_gate(torch.cat([net, net_msg], dim=-1))
            phy_gate = self.phy_gate(torch.cat([phy, phy_msg], dim=-1))
            net = self.net_cross_norm(net + net_gate * net_msg)
            phy = self.phy_cross_norm(phy + phy_gate * phy_msg)
            h = torch.cat([net.squeeze(0), phy.squeeze(0)], dim=0)
...
class HGANTraceDSGCFusionEncoder(nn.Module):
    """Original DSGC/DEWM backbone with explicit typed cross-domain fusion."""
    ...
    def __init__(self, in_channels: int, hidden_channels: int,
        ...
        if use_local_attention:
            ...
            self.local_attention_gates = nn.ModuleList([
                nn.Sequential(
                    nn.Linear(hidden_channels * 2, hidden_channels),
                    nn.Sigmoid(),
                ) for _ in range(num_layers)
    ...
    def encode(self, x, adj, n_net=None):
        h = self.input_proj(x)
        n_net = min(max(int(n_net or 0), 0), h.shape[0])
        ...
        if self.use_dynamic_edge_weights and n_net > 0:
            adj = self.dynamic_edge_module(h, adj, n_net)

        # Preserve each node's own evidence while retaining sparse local edges.
        adj_local = adj + torch.eye(adj.shape[0], dtype=adj.dtype, device=adj.device)
        degree = adj_local.sum(dim=1, keepdim=True).clamp(min=1.0)
        adj_norm = adj_local / degree
        for layer_idx, norm in enumerate(self.local_norms):
            h_agg = torch.matmul(adj_norm, h)
            if self.use_dw_sep:
                g1 = self.dw_g1[layer_idx](h_agg[:, :self._half])
                g2 = self.dw_g2[layer_idx](h_agg[:, self._half:])
                update = self.pw_layers[layer_idx](torch.cat([g1, g2], dim=-1))
            else:
                update = self.gcn_layers[layer_idx](h_agg)
            if self.use_local_attention:
                attention_update, _ = self.local_attention[layer_idx](h, adj_local)
                mix = self.local_attention_gates[layer_idx](
                    torch.cat([update, attention_update], dim=-1)
                )
                update = mix * update + (1.0 - mix) * attention_update
            update = F.gelu(norm(update))
            update = F.dropout(update, p=self.dropout, training=self.training)
            h = h + update
...
class EnhancedTracebackSystem(nn.Module):
    ...
    def __init__(self, gnn_encoder, n_nodes, n_net, n_phy,
                 ...
                 readout_variant='typed', net_feature_dim=0,
                 net_raw_offset=6, detection_encoder=None):
        super().__init__()
        
        self.gnn_encoder = gnn_encoder
        encoder_method = getattr(self.gnn_encoder, 'encode', None)
        self.encoder_accepts_n_net = bool(
            encoder_method is not None
            and 'n_net' in inspect.signature(encoder_method).parameters
        )
        self.n_nodes = n_nodes
        self.n_net = n_net
        self.n_phy = n_phy
        self.latent_dim = latent_dim
        self.input_dim = input_dim
        self.readout_variant = str(readout_variant)
        self.net_feature_dim = int(net_feature_dim)
        self.net_raw_offset = int(net_raw_offset)
        self.detection_encoder = detection_encoder

        # Type-aware graph readout. Network and physical evidence are pooled
        # separately before their agreement and discrepancy are modelled.
        self.net_graph_pool = nn.Sequential(
            nn.Linear(latent_dim, latent_dim),
            nn.LayerNorm(latent_dim),
            nn.Tanh(),
            nn.Linear(latent_dim, 1)
        )
        self.phy_graph_pool = nn.Sequential(
            nn.Linear(latent_dim, latent_dim),
            nn.LayerNorm(latent_dim),
            nn.Tanh(),
            nn.Linear(latent_dim, 1)
        ...
        self.readout_norm = nn.LayerNorm(latent_dim * 6)
        ...
        if self.readout_variant in {'global', 'typed_global'}:
            self.global_graph_pool = nn.Sequential(
                nn.Linear(latent_dim, latent_dim),
                nn.Tanh(),
                nn.Linear(latent_dim, 1),
            )
            ...
            self.global_fusion_logits = nn.Parameter(torch.zeros(n_classes))

        if self.readout_variant == 'dual_encoder':
            if self.detection_encoder is None:
                raise ValueError('dual_encoder readout requires a detection encoder.')
            self.detection_feature_enhance = nn.Sequential(
                nn.Linear(latent_dim, latent_dim),
                nn.LayerNorm(latent_dim),
                nn.GELU(),
                nn.Dropout(0.2),
            ...
            self.detection_fusion_logits = nn.Parameter(
                torch.full((n_classes,), 1.3862944)
            )

        if self.readout_variant in {'context_fusion', 'context_only'}:
            context_dim = self.net_feature_dim + self.n_phy + 2 * self.n_net
            if self.net_feature_dim <= 0:
                raise ValueError('A positive net_feature_dim is required for context readout.')
            ...
            # Per-class gates let physical faults and network attacks rely on
            # different mixtures while keeping both branches auditable.
            self.context_fusion_logits = nn.Parameter(torch.zeros(n_classes))
    ...
    def encode_detection(self, x, adj):
        if self.detection_encoder is None:
            raise RuntimeError('No auxiliary detection encoder is configured.')
        if hasattr(self.detection_encoder, 'encode'):
            z = self.detection_encoder.encode(x, adj)
        elif hasattr(self.detection_encoder, 'get_embeddings'):
            z = self.detection_encoder.get_embeddings(x, adj)
        else:
            z = self.detection_encoder(x, adj)
        return z[0] if isinstance(z, tuple) else z

    def decode_detection(self, z):
        if hasattr(self.detection_encoder, 'decode'):
            return self.detection_encoder.decode(z)
        return torch.sigmoid(torch.matmul(z, z.t()))
    ...
    def forward(self, x, adj, return_traceback=None):
        ...
        # Type-aware cyber-physical readout.
        n_net = min(self.n_net, z_with_residual.shape[0])
        net_z = z_with_residual[:n_net]
        phy_z = z_with_residual[n_net:]
        if net_z.shape[0] == 0:
            net_z = z_with_residual
        if phy_z.shape[0] == 0:
            phy_z = z_with_residual

        net_weights = F.softmax(self.net_graph_pool(net_z).squeeze(-1), dim=0)
        phy_weights = F.softmax(self.phy_graph_pool(phy_z).squeeze(-1), dim=0)
        net_repr = torch.sum(net_weights.unsqueeze(-1) * net_z, dim=0)
        phy_repr = torch.sum(phy_weights.unsqueeze(-1) * phy_z, dim=0)
        net_max = net_z.max(dim=0)[0]
        phy_max = phy_z.max(dim=0)[0]
        interaction = net_repr * phy_repr
        discrepancy = torch.abs(net_repr - phy_repr)
        combined_repr = torch.cat(
            [net_repr, phy_repr, net_max, phy_max, interaction, discrepancy], dim=-1
        )
        combined_repr = self.readout_norm(combined_repr)
        graph_logits = self.classifier(combined_repr)
        logits = graph_logits

        if self.readout_variant in {'global', 'typed_global'}:
            global_weights = F.softmax(
                self.global_graph_pool(z_with_residual).squeeze(-1), dim=0
            )
            global_repr = torch.sum(
                global_weights.unsqueeze(-1) * z_with_residual, dim=0
            )
            global_repr = global_repr + 0.3 * z_with_residual.max(dim=0)[0]
            global_logits = self.global_classifier(global_repr)
            if self.readout_variant == 'global':
                logits = global_logits
            else:
                fusion = torch.sigmoid(self.global_fusion_logits)
                logits = (1.0 - fusion) * graph_logits + fusion * global_logits

        if self.readout_variant == 'dual_encoder':
            detection_z = self.encode_detection(x, adj)
            detection_z = detection_z + self.detection_feature_enhance(detection_z)
            detection_weights = F.softmax(
                self.detection_graph_pool(detection_z).squeeze(-1), dim=0
            )
            detection_repr = torch.sum(
                detection_weights.unsqueeze(-1) * detection_z, dim=0
            )
            detection_repr = detection_repr + 0.3 * detection_z.max(dim=0)[0]
            detection_logits = self.detection_classifier(detection_repr)
            fusion = torch.sigmoid(self.detection_fusion_logits)
            logits = (1.0 - fusion) * graph_logits + fusion * detection_logits

        if self.readout_variant in {'context_fusion', 'context_only'}:
            role = x[:n_net, 4]
            active = (torch.abs(role) > 0.5).to(x.dtype).unsqueeze(-1)
            raw_start = self.net_raw_offset
            raw_end = raw_start + self.net_feature_dim
            net_raw = x[:n_net, raw_start:raw_end]
            active_count = active.sum().clamp(min=1.0)
            net_context = torch.sum(net_raw * active, dim=0) / active_count

            phy_context = x[n_net:n_net + self.n_phy, 0]
            if phy_context.shape[0] < self.n_phy:
                phy_context = F.pad(phy_context, (0, self.n_phy - phy_context.shape[0]))
            source_identity = torch.clamp(role, min=0.0, max=1.0)
            destination_identity = torch.clamp(-role, min=0.0, max=1.0)
            context = torch.cat(
                [net_context, phy_context, source_identity, destination_identity],
                dim=0,
            )
            context_logits = self.context_classifier(self.context_norm(context))
            if self.readout_variant == 'context_only':
                logits = context_logits
            else:
                fusion = torch.sigmoid(self.context_fusion_logits)
                logits = (1.0 - fusion) * graph_logits + fusion * context_logits
...
class TaskDecoupledTraceSystem(nn.Module):
    """Use separate lightweight encoders for detection and root localization.

    The detector supplies graph-level class logits, while the typed HGAN
    localizer supplies node embeddings to the shared exact-root head. An
    optional validation-locked convex probability fusion lets the localizer's
    classifier contribute to detection without changing root-cause scores.
    """

    def __init__(self, detector, localizer, detection_localizer_weight=0.0):
        super().__init__()
        self.detector = detector
        self.localizer = localizer
        self.detection_localizer_weight = min(
            max(float(detection_localizer_weight), 0.0), 1.0
        )
        self.n_nodes = getattr(localizer, 'n_nodes', getattr(detector, 'n_nodes', 0))
        self.latent_dim = getattr(localizer, 'latent_dim', 0)

    def encode(self, x, adj):
        return self.localizer.encode(x, adj)

    def get_embeddings(self, x, adj):
        return self.encode(x, adj)

    def decode(self, z):
        return self.localizer.decode(z)

    def forward(self, x, adj, return_traceback=False):
        detection_out = self.detector(x, adj, return_traceback=False)
        detection_logits = (
            detection_out['anomaly_logits']
            if isinstance(detection_out, dict) else detection_out
        )

        local_out = None
        if self.detection_localizer_weight > 0 or return_traceback:
            local_out = self.localizer(
                x, adj, return_traceback=return_traceback
            )
        if self.detection_localizer_weight > 0:
            local_logits = (
                local_out['anomaly_logits']
                if isinstance(local_out, dict) else local_out
            )
            weight = self.detection_localizer_weight
            mixed_probs = (
                weight * F.softmax(local_logits, dim=-1)
                + (1.0 - weight) * F.softmax(detection_logits, dim=-1)
            )
            detection_logits = torch.log(mixed_probs.clamp_min(1e-12))

        if not return_traceback:
            return {'anomaly_logits': detection_logits}

        if isinstance(local_out, dict):
            local_out = dict(local_out)
            local_out['anomaly_logits'] = detection_logits
            return local_out
        return {
            'anomaly_logits': detection_logits,
            'embeddings': self.encode(x, adj),
        }


class DeepEnsembleDetector(nn.Module):
    """Average class probabilities from independently trained detectors."""
    ...
    def __init__(self, detectors):
        ...
        self.n_nodes = getattr(detectors[0], 'n_nodes', 0)
        self.latent_dim = getattr(detectors[0], 'latent_dim', 0)

    def forward(self, x, adj, return_traceback=False):
        probabilities = []
        for detector in self.detectors:
            out = detector(x, adj, return_traceback=False)
            logits = out['anomaly_logits'] if isinstance(out, dict) else out
            probabilities.append(F.softmax(logits, dim=-1))
        mean_probs = torch.stack(probabilities, dim=0).mean(dim=0)
        return {'anomaly_logits': torch.log(mean_probs.clamp_min(1e-12))}


class PriorAdjustedDetector(nn.Module):
    """Apply a fixed class-prior correction to detector probabilities."""

    def __init__(self, detector, class_priors, alpha):
        super().__init__()
        priors = torch.as_tensor(class_priors, dtype=torch.float32)
        if priors.ndim != 1 or torch.any(priors <= 0):
            raise ValueError('class_priors must be a positive one-dimensional vector.')
        self.detector = detector
        self.alpha = float(alpha)
        self.register_buffer('prior_scale', priors.pow(self.alpha))
        self.n_nodes = getattr(detector, 'n_nodes', 0)
        self.latent_dim = getattr(detector, 'latent_dim', 0)

    def encode(self, x, adj):
        return self.detector.encode(x, adj)

    def forward(self, x, adj, return_traceback=False):
        out = self.detector(x, adj, return_traceback=False)
        logits = out['anomaly_logits'] if isinstance(out, dict) else out
        probs = F.softmax(logits, dim=-1) * self.prior_scale
        probs = probs / probs.sum(dim=-1, keepdim=True).clamp_min(1e-12)
        return {'anomaly_logits': torch.log(probs.clamp_min(1e-12))}


class ClassScaleAdjustedDetector(nn.Module):
    """Apply validation-locked positive scales to class probabilities."""

    def __init__(self, detector, class_scales):
        super().__init__()
        scales = torch.as_tensor(class_scales, dtype=torch.float32)
        if scales.ndim != 1 or torch.any(scales <= 0):
            raise ValueError('class_scales must be a positive one-dimensional vector.')
        self.detector = detector
        self.register_buffer('class_scales', scales)
        self.n_nodes = getattr(detector, 'n_nodes', 0)
        self.latent_dim = getattr(detector, 'latent_dim', 0)
    ...
    def forward(self, x, adj, return_traceback=False):
        out = self.detector(x, adj, return_traceback=False)
        logits = out['anomaly_logits'] if isinstance(out, dict) else out
        probs = F.softmax(logits, dim=-1) * self.class_scales
        probs = probs / probs.sum(dim=-1, keepdim=True).clamp_min(1e-12)
        return {'anomaly_logits': torch.log(probs.clamp_min(1e-12))}


class TemperatureScaledDetector(nn.Module):
    """Apply a fixed positive temperature to detector logits."""

    def __init__(self, detector, temperature):
        super().__init__()
    ...
    def forward(self, x, adj, return_traceback=False):
        out = self.detector(x, adj, return_traceback=False)
        logits = out['anomaly_logits'] if isinstance(out, dict) else out
        return {'anomaly_logits': logits / self.temperature}
    ...
    for epoch in range(epochs):
        ...
        for batch_start in range(0, n_samples, batch_size):
            ...
            for t in batch_indices:
                ...
                if getattr(encoder, 'detection_encoder', None) is not None:
                    detection_z = encoder.encode_detection(x, adj_tensor)
                    detection_recon = encoder.decode_detection(detection_z)
                    detection_edge_loss = F.binary_cross_entropy(
                        detection_recon, adj_tensor
                    )
                    edge_loss = 0.5 * (edge_loss + detection_edge_loss)
...
def finetune_supervised(model, data, graph_builder, device, epochs=40, lr=0.001,
                        ...
                        use_phy_chain=True, use_net_edges=True,
                        verbose=True, model_name="Model",
                        encoder_lr_scale=0.1, class_weight_power=1.0,
                        apply_class_boosts=True, oversample=True,
                        label_smoothing=0.0, focal_gamma=0.0,
                        early_stopping_patience=15,
                        temporal_recency_strength=0.0, selection_data=None,
                        restore_best=True, distillation_teacher=None,
                        distillation_logits=None,
                        distillation_alpha=0.0, distillation_temperature=2.0,
                        joint_root_head=None, joint_root_weight=0.0,
                        joint_root_stride=1):
    """
    阶段2: 有监督微调 - 使用标签训练分类头
    ...
    for name, param in model.named_parameters():
        if (
            ...
            or 'causal_traceback' in name or 'feature_enhance' in name
            or 'readout_norm' in name
        ):
    ...
    # 计算类别权重（激进式类别平衡）
    val_data = selection_data if selection_data is not None else data.get('_val_data', data)
    y = data['y']
    class_counts = np.bincount(y)
    
    # The exponent makes class balancing tunable without changing the split.
    class_weights = (1.0 / (class_counts + 1e-6)) ** float(class_weight_power)
    ...
    nm_idx = None
    if apply_class_boosts and 'NM' in label_names:
    ...
    # PS类别也是少数类，需要增强
    if apply_class_boosts and 'PS' in label_names:
    ...
    # 差异化学习率优化器
    optimizer_groups = [
        {'params': encoder_params, 'lr': lr * float(encoder_lr_scale)},
        {'params': classifier_params, 'lr': lr}      # 分类器用正常学习率
    ]
    root_weight = max(0.0, float(joint_root_weight))
    root_stride = max(1, int(joint_root_stride))
    if joint_root_head is not None and root_weight > 0:
        joint_root_head.to(device)
        optimizer_groups.append({'params': list(joint_root_head.parameters()), 'lr': lr})
    optimizer = torch.optim.AdamW(optimizer_groups, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2)

    kd_alpha = min(max(float(distillation_alpha), 0.0), 1.0)
    kd_temperature = max(float(distillation_temperature), 1e-3)
    cached_teacher_logits = None
    if distillation_logits is not None and kd_alpha > 0:
        cached_teacher_logits = torch.as_tensor(
            distillation_logits, dtype=torch.float32, device=device
        )
        if cached_teacher_logits.ndim != 2 or cached_teacher_logits.shape[0] != len(y):
            raise ValueError(
                'distillation_logits must have shape [number of training samples, classes].'
            )
        distillation_teacher = None
    elif distillation_teacher is not None and kd_alpha > 0:
        distillation_teacher.to(device)
        distillation_teacher.eval()
        for param in distillation_teacher.parameters():
            param.requires_grad_(False)
    else:
        distillation_teacher = None
        kd_alpha = 0.0

    if verbose and kd_alpha > 0 and (
        distillation_teacher is not None or cached_teacher_logits is not None
    ):
        print(
            f"     Distillation: alpha={kd_alpha:.2f}, "
            f"temperature={kd_temperature:.2f}, "
            f"source={'cached logits' if cached_teacher_logits is not None else 'live teacher'}"
        )
    if verbose and joint_root_head is not None and root_weight > 0:
        print(
            f"     Joint root supervision: weight={root_weight:.3f}, "
            f"stride={root_stride}"
        )
    ...
    # Weight recent observations more strongly within each class without using
    # validation/test labels. A value of zero preserves the original objective.
    recency_weights = np.ones(n_samples, dtype=np.float32)
    recency_strength = max(0.0, float(temporal_recency_strength))
    if recency_strength > 0:
        for cls_id in range(len(class_counts)):
            cls_indices = np.where(y == cls_id)[0]
            if len(cls_indices) <= 1:
                continue
            progress = np.linspace(0.0, 1.0, len(cls_indices), dtype=np.float32)
            cls_weights = np.exp(recency_strength * (progress - 1.0))
            cls_weights /= max(float(cls_weights.mean()), 1e-8)
            recency_weights[cls_indices] = cls_weights
        if verbose:
            print(
                f"     🕒 Temporal recency strength={recency_strength:.2f}, "
                f"sample-weight range={recency_weights.min():.3f}--{recency_weights.max():.3f}"
            )
    ...
    if oversample:
        for cls_id in range(len(class_counts)):
            ...
            # Preserve the legacy NM cap only when the legacy manual boosts are enabled.
            if apply_class_boosts and nm_idx is not None and cls_id == nm_idx:
                repeat_times = max(1, nm_target // len(cls_indices))
                oversampled_indices.extend(np.repeat(cls_indices, repeat_times).tolist())
            elif class_counts[cls_id] < minority_target:
                repeat_times = max(1, minority_target // len(cls_indices))
                oversampled_indices.extend(np.repeat(cls_indices, repeat_times).tolist())
            else:
                oversampled_indices.extend(cls_indices.tolist())
    else:
        oversampled_indices.extend(np.arange(n_samples).tolist())
    ...
    for epoch in range(epochs):
        ...
        for batch_start in range(0, len(oversampled_indices), batch_size):
            ...
            for t in batch_indices:
                ...
                ce_loss = F.cross_entropy(
                    logits, label, weight=class_weights,
                    label_smoothing=float(label_smoothing),
                    # Each graph is evaluated separately inside the batch loop.
                    # ``mean`` would divide by that graph's target-class weight
                    # and cancel class weighting for a one-sample tensor.
                    reduction='none',
                )
                ...
                if gamma > 0:
                    ...
                    cls_loss = (((1.0 - target_prob) ** gamma) * ce_loss).sum()
                else:
                    cls_loss = ce_loss.sum()
                if distillation_teacher is not None or cached_teacher_logits is not None:
                    if cached_teacher_logits is not None:
                        teacher_logits = cached_teacher_logits[t].unsqueeze(0)
                    else:
                        with torch.no_grad():
                            teacher_out = distillation_teacher(
                                x, adj_tensor, return_traceback=False
                            )
                            teacher_logits = (
                                teacher_out['anomaly_logits']
                                if isinstance(teacher_out, dict) else teacher_out
                            )
                    ...
                    sample_loss = (
                        (1.0 - kd_alpha) * cls_loss + kd_alpha * kd_loss
                    ) * float(recency_weights[t])
                else:
                    sample_loss = cls_loss * float(recency_weights[t])

                needs_joint_root = (
                    joint_root_head is not None
                    and root_weight > 0
                    and int(y[t]) > 0
                    and ((total_samples + epoch) % root_stride == 0)
                    and isinstance(out, dict)
                    and 'embeddings' in out
                )
                if needs_joint_root:
                    root_target = build_root_target(t, data, graph_builder, device)
                    if root_target is not None:
                        root_scores = joint_root_head(out['embeddings'])
                        ...
                        root_weights[root_target > 0] = max(
                            5.0,
                            graph_builder.n_nodes / max(root_target.sum().item(), 1.0),
                        )
                        root_loss = F.binary_cross_entropy(
                            root_scores, root_target, weight=root_weights
                        )
                        sample_loss = sample_loss + root_weight * root_loss
            ...
            if joint_root_head is not None and root_weight > 0:
                clip_params.extend(joint_root_head.parameters())
        ...
        finetune_history.append({
            'epoch': epoch + 1,
            'loss': float(avg_loss),
            'selection_accuracy': float(eval_acc),
            'selection_f1': float(eval_f1),
        })
        model.train()
        if joint_root_head is not None and root_weight > 0:
            joint_root_head.train()
        ...
        if eval_f1 > best_f1 or (eval_f1 == best_f1 and eval_acc > best_acc):
            ...
            if joint_root_head is not None and root_weight > 0:
                best_root_state = {
                    k: v.cpu().clone() for k, v in joint_root_head.state_dict().items()
                }
            best_epoch = epoch + 1
            patience_counter = 0
        else:
    ...
    if restore_best and best_state is not None:
        ...
        if best_root_state is not None:
            joint_root_head.load_state_dict(
                {k: v.to(device) for k, v in best_root_state.items()}
            )

    model.finetune_history = finetune_history
    model.finetune_best_epoch = best_epoch
    
    if verbose:
        if restore_best:
            print(
                f"     ✅ 微调完成! Best epoch: {best_epoch}, "
                f"Best Acc: {best_acc:.2f}%, Best F1: {best_f1:.2f}%"
            )
        else:
            print(f"     ✅ 固定轮数重训完成! Retained epoch: {len(finetune_history)}")
...
def run_ablation_and_comparison(data, graph_builder, device, epochs=30, lr=0.002, outdir='traceback_results'):
    ...
    架构特点:
        ...
        深度可分离图卷积 × L (Depthwise Separable Graph Conv, DSGC)
           ↓
        双向信息物理注意力 (Bidirectional Cyber-Physical Attention)
           ↓
    ...
    print(f"     - w/o Cross-Attn : 去除双向信息物理注意力")
    ...
    print("\n📌 [1/13] 训练主模型: Ours (typed DSGC cyber-physical fusion)...")
    hgt_trace_full = HGANTraceDSGCFusionEncoder(
    ...
    hgt_trace_wo_dwsep = HGANTraceDSGCFusionEncoder(
        in_channels=node_dim, hidden_channels=hidden_dim, latent_channels=out_dim,
        num_layers=2, n_heads=4, dropout=0.15,
        use_cross_attention=True,
        use_type_adapters=True,
    ...
    # ==================== 3. 消融模型: w/o Cross-Attn ====================
    print("\n📌 [3/13] 训练消融模型: w/o Cross-Attn...")
    hgt_trace_wo_ts = HGANTraceDSGCFusionEncoder(
        in_channels=node_dim, hidden_channels=hidden_dim, latent_channels=out_dim,
        num_layers=2, n_heads=4, dropout=0.15,
    ...
    model_wo_ts = train_two_stage(model_wo_ts, data, graph_builder, device,
                                   ...
                                   use_dynamic_edges=False, use_cross_layer=False,
                                   use_phy_chain=True, use_net_edges=True,
                                   verbose=False, model_name="w/o Cross-Attn")
    results['w/o Cross-Attn'] = evaluate_model(model_wo_ts, data, graph_builder, device,
                                                    use_dynamic_edges=False, use_cross_layer=False,
                                                    use_phy_chain=True, use_net_edges=True)
    models_dict['w/o Cross-Attn'] = model_wo_ts
    print(f"   ✓ Accuracy: {results['w/o Cross-Attn']['accuracy']:.2f}%, F1-macro: {results['w/o Cross-Attn']['f1']:.2f}%, F1-weighted: {results['w/o Cross-Attn'].get('f1_weighted', 0):.2f}%")

    # ==================== 4. 消融模型: w/o Dynamic Edge Weighting (去除动态边权重模块) ====================
    print("\n📌 [4/13] 训练消融模型: w/o Dynamic Edge Weighting (去除动态边权重模块)...")
    hgt_trace_wo_dynamic_edge_weighting = HGANTraceDSGCFusionEncoder(
        in_channels=node_dim, hidden_channels=hidden_dim, latent_channels=out_dim,
        num_layers=2, n_heads=4, dropout=0.15,
        use_cross_attention=True,
        use_type_adapters=True,
...
def train_main_model_only(data, graph_builder, device, epochs=2, lr=0.002, outdir='traceback_results'):
    ...
    print("\n" + "=" * 70)
    print("📌 只训练主模型: Ours (typed DSGC cyber-physical fusion)")
    print("=" * 70)
    print("   保留模块: DSGC, Dynamic Edge Weighting, bidirectional cross-attention")
    ...
    encoder = HGANTraceDSGCFusionEncoder(
        in_channels=node_dim, hidden_channels=hidden_dim, latent_channels=out_dim,
        num_layers=2, n_heads=4, dropout=0.15,
        use_cross_attention=True,
        use_type_adapters=True,
